...
static void hall_device_irq_work(struct work_struct *work)
{
    ...
    {
	input_report_switch(chip->idev, SW_LID, hall_device_state==1 ? 1 : 0);
                  ...
                  const struct i2c_device_id *id)
{
    ...
    set_bit(SW_LID, 	chip->idev->swbit);
    ...
    input_report_switch(chip->idev, SW_LID, gpio_get_value(chip->irq.irq_pin) ? 0 : 1);
    input_sync(chip->idev);
