...
static int cyttsp5_fw_calibration_attention(struct cyttsp5_device *ttsp)
{
	...
	cyttsp5_unsubscribe_attention(ttsp, CY_ATTEN_IRQ,
		cyttsp5_fw_calibration_attention, CY_MODE_OPERATIONAL);
...
static int ztemt_cyttsp5_check_firmware_version_builtin(struct cyttsp5_device *ttsp)
{
	struct device *dev = &ttsp->dev;
	struct cyttsp5_loader_data *data = dev_get_drvdata(dev);
...
//	upgrade = cyttsp5_check_firmware_version_builtin(ttsp, fw);
    upgrade = ztemt_cyttsp5_check_firmware_version_builtin(ttsp);
...
static int upgrade_firmware_from_builtin(struct cyttsp5_device *ttsp)
{
    ...
    if (data->si == NULL) {
    	...
    	retval = request_firmware_nowait(THIS_MODULE, FW_ACTION_HOTPLUG,
    			data->si->cy_fw_file_name, dev, GFP_KERNEL, ttsp,
...
/*** ZTEMT Added by luochangyang, 2013/09/12 ***/
static ssize_t cyttsp5_fw_upgrade_flag_show(struct device *dev, struct device_attribute *attr, char *buf)
{	
	return snprintf(buf, PAGE_SIZE,"%d\n",upgrade_flag);
}
static DEVICE_ATTR(fw_upgrade_flag, 0664, cyttsp5_fw_upgrade_flag_show, NULL);

static ssize_t cyttsp5_manual_cali_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct cyttsp5_loader_data *data = dev_get_drvdata(dev);
