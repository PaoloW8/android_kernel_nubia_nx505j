...
#define CY_HID_RESET_TIMEOUT			1500  //5000  too long, seize the day. Modify by luochangyang, 2013/11/12
...
#define CY_HID_OUTPUT_BL_INITIATE_BL_TIMEOUT	20000       //8000
...
struct cyttsp5_core_data {
	...
	bool irq_enabled;
	bool irq_wake;
	u8 easy_wakeup_gesture; //hyuc: for easy wakeup
	bool wake_initiated_by_device; //hyuc: for easy wakeup
...
/* ZTEMT Added by luochangyang, 2013/09/06 */
    int touch_mode;     //for hover switch
#if defined(CONFIG_FB)
		...
		struct cyttsp5_hid_output *hid_output)
{
    ...
    /*ZTEMT Added by luochangyang, 2014/01/09*/
#if 0
		...
		struct device_attribute *attr, char *buf)
{
	...
	rc = cyttsp5_hid_output_get_param(cd, param_id, &mode0);
	if (rc < 0) {
		dev_err(dev, "%s: cyttsp5_hid_output_get_param failed r=%d\n",
			__func__, rc);
		
		return snprintf(buf, CY_MAX_PRBUF_SIZE, "0x%04X\n", rc);
	}

	rc = cyttsp5_hid_output_get_param(cd, 0x5a, &mode1);
	...
	return snprintf(buf, CY_MAX_PRBUF_SIZE, "mode0 = 0x%04X, mode1 = 0x%04X\n", mode0, mode1);
}
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	/*Switch touch mode*/
	if (value == 1 || value == 2) {
		/*01: Enter in Holster, 02: Exit Holster*/
		rc = cyttsp5_hid_output_set_param(cd, 0x5a, value);
	} else {
	    /*00:Finger Only, 03: Auto glove; 15: All feature*/
		rc = cyttsp5_hid_output_set_param(cd, param_id, value);
	}

	if (rc < 0) {
		dev_err(dev, "%s: cyttsp5_hid_output_set_param failed r=%d\n",
			__func__, rc);
...
//	si->cydata.fw_ver_conf = config_ver;
    si->fw_ver_ic = config_ver;

	si->update_flag = cd->pdata->check_version(si, cd->dev);
	dev_info(cd->dev, "%s: FW in TP:0x%04X  FW in driver:0x%04X\n", 
        __func__, si->fw_ver_ic, si->fw_ver_dr);
    dev_info(cd->dev, "%s: update_flag:%d  Firmware File Name %s\n", 
        __func__, si->update_flag, si->cy_fw_file_name);
/***ZTEMT END***/
...
//	cd->wake_initiated_by_device = 1;
	...
	/*** ZTEMT Added  by luochangyang, 2014/01/02 ***/
...
#elif defined(CONFIG_HAS_EARLYSUSPEND)
static void cyttsp5_core_early_suspend(struct early_suspend *h)
{	int rc;
	struct cyttsp5_core_data *cd = container_of(h, struct cyttsp5_core_data, es);
	struct device *dev = cd->dev;
	dev_info(dev, "ztemt %s\n", __func__);
	rc = cyttsp5_core_sleep(cd);
	if (rc < 0) {
		dev_err(dev, "%s: Error on sleep\n", __func__);
	}
}
static void cyttsp5_core_late_resume(struct early_suspend *h)
{	
	int rc;
	struct cyttsp5_core_data *cd = container_of(h, struct cyttsp5_core_data, es);
	struct device *dev = cd->dev;
		...
		struct device_attribute *attr, char *buf)
{
	...
	return sprintf(buf,
		...
		"%s: 0x%02X\n"
		"%s: 0x%02X\n",
		"Cyttsp5 Firmware Version in TP IC ",cd->sysinfo.fw_ver_ic,
		"Cyttsp5 Firmware Version in Driver",cd->sysinfo.fw_ver_dr,
...
static ssize_t cyttsp5_hw_reset_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct cyttsp5_core_data *cd = dev_get_drvdata(dev);
	int rc;
...
static struct device_attribute attributes[] = {
	...
	__ATTR(hw_reset, 0664, cyttsp5_hw_reset_show, cyttsp5_hw_reset_store),
	...
	__ATTR(touch_mode, 0664,
		cyttsp5_touch_mode_show,
...
static int cyttsp5_core_probe(struct cyttsp5_core *core)
{
	...
	setup_timer(&cd->watchdog_timer, cyttsp5_watchdog_timer,
			(unsigned long)cd);
#if 0
    cd->easy_wakeup_gesture = CY_CORE_EWG_TAP_TAP;  //Added by luochangyang, 2013/10/31
#else
    cd->easy_wakeup_gesture = CY_CORE_EWG_NONE;  //Added by luochangyang, 2013/10/31
#endif
...
/* ZTEMT Added by luochangyang, 2013/09/06 */
#if defined(CONFIG_FB)
    cd->fb_notif.notifier_call = cyttsp5_fb_notifier_callback;
    ...
    cd->es.suspend = cyttsp5_core_early_suspend;
    cd->es.resume = cyttsp5_core_late_resume;
...
/* ZTEMT END */
	...
	/*luochangyang 2014/03/27 Default No glove*/
	rc = cyttsp5_hid_output_set_param(cd, 0xd0, 0);
	if (rc < 0)
		dev_err(cd->dev, "%s: cyttsp5_hid_output_set_param failed r=%d\n",
			__func__, rc);
	/*luochangyang END*/
