...
/****************************************************************
	...
	mobile: 13244776877
-----------------------------------------------------------------*/
#define PLUG_CAL_T37_VERSION 0x0110
/*----------------------------------------------------------------
0.108
...
enum{
	...
	SC_TEMP,
	SC_BUFFER_NUMBER,
	SC_BUFFER_INVALID = SC_BUFFER_NUMBER
};

/*
static char *t37_sc_name[SC_BUFFER_NUMBER] = {
...
#define T37_FLAG_T8_SET				(1<<4)
#define T37_FLAG_T65_SET				(1<<5)
...
#define T37_FLAG_SC_SIGNAL			(1<<18)
#define T37_FLAG_SC_IRON				(1<<19)
...
#define SHIFT_SC_DRIFT_SLOW_THLD 8
#define SHIFT_SC_DRIFT_FAST_THLD 7

//#define SC_DRIFT_ITEMS 1

struct t37_observer{
	void *obs_buf;

	s16 *buf[T37_BUFFER_NUMBER];
	s16 *sc_buf[SC_BUFFER_NUMBER];
...
#define GRAD_STATE_LIST 2
	...
	struct surface_count sf_sc_cnt[SC_BUFFER_NUMBER];
...
#define WK_KEEP_NO_CALIBRAION_AT_RESUME	(1<<29)

//------selfcap --------
#define ACCUMULATOR_SC_X		(1<<3)
#define ACCUMULATOR_SC_Y		(1<<4)

	unsigned long sc_acc_flag;

#define NUM_SC_LEVEL_LIST 8
	int *sc_accu_level_array;
	int sc_level_step[SC_NUM];
#define ACCUMULATOR_SC_DRIFT_FAST	(1<<12)
#define ACCUMULATOR_SC_DRIFT_NORM	(1<<13)
	int *sc_drift_coef_array;

	int sc_accumulator_low;
	int sc_accumulator_high;

	int shift_sc_drift_low;  //value base on step
	int shift_sc_drift_high;
...
int get_t8_state(unsigned long step, unsigned long flag, unsigned long pl_flag, int step_t8, int num_t8_config)
{
	...
	if (test_flag(T37_STEP_0_IN,&step)) {
		...
		if (test_flag(T37_FLAG_PHONE_ON,&flag))
			...
			if (test_flag(T37_FLAG_T8_SET,&flag)) {
...
void print_range_surface(int sc,const char *prefix_str, s16 *buf,const struct range_t *r)
{
	if (r->end >= r->start)
		print_dec16_buf(KERN_INFO, prefix_str, buf + r->start, r->end -r->start + 1);
}
...
#if defined(CONFIG_MXT_CAL_T37_SC_WORKAROUND)
static int merge_range(s16 *dst_buf,s16 *src0_buf, s16 *src1_buf,const struct range_t *r,int flag)
{
	int pos;
	s16 d0,d1;

	if (flag & SURFACE_MERGE_COPY)
		memcpy(dst_buf + r->start, src0_buf + r->start,
				(r->end - r->start + 1) * sizeof(dst_buf[0]));
	else
...
#if defined(CONFIG_MXT_CAL_T37_SC_WORKAROUND)
static int selfcap_percent(s16 sc_val,s16 sc_step,int *sc_level_list,int num_sc_level_list)
{
	int j;
	int percent = 0;

	sc_val = abs(sc_val);
	if (sc_val >= sc_step) { //sc_level_list store as ascending order
		for (j = 0; j < num_sc_level_list; j++) {
			if (sc_val >= sc_step) 
				sc_val -= sc_step;
			else
				break;
		...
		j--;
		percent = sc_level_list[j];
		if (sc_val && sc_val < sc_step)
			percent += (sc_level_list[j + 1] - sc_level_list[j] )*sc_val / sc_val;
		
#if (DBG_LEVEL > 1)
		printk(KERN_INFO "[mxt]val %hd step %hd %d[%hu] percent %d\n",
			sc_val, sc_step , j, sc_level_list[j],percent);
#endif
	}
...
static int caculate_selfcap_accumulator(s16 *sc_buf,const struct range_t *r,int *sc_level_list,int num_sc_level_list,s16 sc_step)
{
	int pos;
	int accu, accumulator= 0;
	...
	for (pos = r->start; pos < r->end; pos++) {
		accu = selfcap_percent(sc_buf[pos], sc_step, sc_level_list, num_sc_level_list);
		accumulator += accu;
		#if (DBG_LEVEL > 1)
		if (accu) {
			printk(KERN_INFO "[mxt][%d] step %hd accu %d sum %d\n",
				sc_buf[pos],sc_step, accu, accumulator);
		}
		#endif
	}
	return accumulator;
...
static int caculate_selfcap(s16 *dst_buf,s16 *src_buf, s16 *cache , const struct range_t *r)
{
	int pos;
	...
	for (pos = r->start; pos <= r->end; pos++) {
		if (src_buf[pos]) {
			if (cache)
				dst_buf[pos] = src_buf[pos] - cache[pos];
			else
...
//output: max_array: 
static int sc_signal_cache_renew(s16 *sc_buf, s16 *sc_target, const struct range_t *r, int sc_drift_coef, s16 low_limit, s16 high_limit)
{
	int pos;
	s16 val,diff,val_t;
...
static void selfcap_merge_range(struct plugin_cal *p, int dst, int sc0, int sc1, int flag,unsigned long sc_acc_flag)
{
	const struct mxt_config *dcfg = p->dcfg;
	struct t37_observer *obs = p->obs;
	s16 *dst_buf,*sc0_buf,*sc1_buf;

	sc0_buf = obs->sc_buf[sc0];
	sc1_buf = obs->sc_buf[sc1];
	dst_buf = obs->sc_buf[dst];

	if (test_flag(ACCUMULATOR_SC_X, &sc_acc_flag))
		merge_range(dst_buf,sc0_buf,sc1_buf,&dcfg->r[SC_X],flag);

	if (test_flag(ACCUMULATOR_SC_Y, &sc_acc_flag)) 
		merge_range(dst_buf,sc0_buf,sc1_buf,&dcfg->r[SC_Y],flag);
}


static void selfcap_signal_cache_renew(struct plugin_cal *p, int sc, int target,int sc_drift_coef,unsigned long sc_acc_flag)
{
	const struct mxt_config *dcfg = p->dcfg;
	struct t37_config *cfg = p->cfg;
	struct t37_observer *obs = p->obs;
	int low_limit, high_limit;
	s16 *sc_buf,*target_buf;

	sc_buf = obs->sc_buf[sc];
	target_buf = obs->sc_buf[target];

	if (test_flag(ACCUMULATOR_SC_X, &sc_acc_flag)) {
		low_limit = cfg->sc_level_step[SC_X] >> cfg->shift_sc_drift_low;
		high_limit = cfg->sc_level_step[SC_X] >> cfg->shift_sc_drift_high;
		sc_signal_cache_renew(sc_buf,target_buf,&dcfg->r[SC_X],sc_drift_coef,low_limit,high_limit);
	}

	if (test_flag(ACCUMULATOR_SC_Y, &sc_acc_flag)) {
		low_limit = cfg->sc_level_step[SC_Y] >> cfg->shift_sc_drift_low;
		high_limit = cfg->sc_level_step[SC_Y] >> cfg->shift_sc_drift_high;
		sc_signal_cache_renew(sc_buf,target_buf,&dcfg->r[SC_Y],sc_drift_coef,low_limit,high_limit);
	}
}

static int mxt_surface_sc_statistics(struct plugin_cal *p, unsigned long pl_flag,int diff, int sc,  int cache,
					int *sc_level_list, int num_sc_level_list,int drift,unsigned long sc_acc_flag)
{
	const struct mxt_config *dcfg = p->dcfg;
	struct device *dev = dcfg->dev;
	struct t37_observer *obs = p->obs;
	...
	s16 *src_buf,*diff_buf,*cache_buf;
	int sc_step,accu[SC_NUM],accumulator = 0;
	int i;

	src_buf = obs->sc_buf[sc];
	diff_buf = obs->sc_buf[diff];
	if (cache < SC_BUFFER_NUMBER)
		cache_buf = obs->sc_buf[cache];
	else
		cache_buf = NULL;
	...
	for (i = 0; i < SC_NUM; i++) {
		accu[i] = 0;
		if (test_flag(ACCUMULATOR_SC_X, &sc_acc_flag)) {
			print_range_surface(sc,NULL,src_buf,&dcfg->r[i]);
			caculate_selfcap(diff_buf,src_buf,cache_buf,&dcfg->r[i]);
			print_range_surface(sc,NULL,src_buf,&dcfg->r[i]);

			sc_step = cfg->sc_level_step[i];
			accu[i] = caculate_selfcap_accumulator(diff_buf,&dcfg->r[i],
					sc_level_list,num_sc_level_list,sc_step);
		}
	}
	
	accumulator = accu[SC_X] + accu[SC_Y];

#if (DBG_LEVEL > 0)
	dev_info2(dev, "sc %d flag 0x%lx accumulator(%d, x %d y %d)\n",sc,sc_acc_flag,accumulator,accu[SC_X],accu[SC_Y]);
#endif
	return accumulator;
}
#endif
...
static int mxt_surface_aquire_and_compare(struct plugin_cal *p, int sf, int compare,s16 diff_threshold_t9,s16 diff_threshold_t15,int no_wait, unsigned long flag)
{
	...
	do {
		...
		ret = mxt_surface_page_aquire(pl->dev,buf,command,&surface,&range,sf_info->y, &pos, 1, no_wait ? 0 : 5);
		if (ret < 0) {
...
static int mxt_surface_aquire_and_compare_auto(struct plugin_cal *p,int sf,int compare,s16 diff_threshold_t9,s16 diff_threshold_t15,int no_wait, unsigned long flag)
{
	...
	if (sf_info->x < x_size) {
		...
		range.y0 = dcfg->m[MX_POS].y0;
		range.y1 = dcfg->m[MX_POS].y0 + area.y1;
		ret = mxt_surface_page_aquire(pl->dev,buf,cmd,&surface, &range, sf_info->y,&pos ,1, no_wait ? 0 : 5);
		if (ret < 0) {
...
static int mxt_surface_aquire_reset(struct plugin_cal *p, int sf,int no_wait)
{
	...
	return mxt_diagnostic_reset_page(pl->dev, command, 1, no_wait ? 0 : 5);
}
...
static void plugin_cal_t37_hook_t9_t100(struct plugin_cal *p, int id, int x, int y, u8 status)
{
	struct t37_observer *obs = p->obs;
	struct t37_config *cfg = p->cfg;
	int distance;
...
static int plugin_cal_t37_check_and_calibrate(struct plugin_cal *p, bool check_sf, bool resume, unsigned long pl_flag)
{
	const struct mxt_config *dcfg = p->dcfg;
	struct plug_interface *pl = container_of(dcfg, struct plug_interface, init_cfg);
	struct device *dev = dcfg->dev;
	...
	if (ret) {
		if (resume && test_flag(WK_KEEP_NO_CALIBRAION_AT_RESUME,&cfg->grad_acc_flag)) {
			dev_info2(dev, "mxt t37 check and calibration, dummy cal pl_flag=0x%lx\n",pl_flag);
			p->emu_plugin_hook_t6(p->dev, MXT_T6_STATUS_CAL);
			p->emu_plugin_hook_t6(p->dev, 0);
		}else {
			dev_info2(dev, "mxt t37 check and calibration, calibratting pl_flag=0x%lx\n",pl_flag);
			state = get_t8_state(T37_STEP_0_ENTER, obs->flag, pl_flag, 0, 0);
			p->set_t8_cfg(p->dev, state, 0);
			if (state >= T8_WEAK_PALM || state == T8_HALT)
				clear_flag(T37_FLAG_T8_SET, &obs->flag);
			else
				set_flag(T37_FLAG_T8_SET, &obs->flag);
			p->set_t6_cal(p->dev);
		}
	}
...
static int mxt_proc_step_1_msg(struct plugin_cal *p,unsigned long pl_flag)
{
	...
	if (test_and_clear_flag(T37_FLAG_RESUME|T37_FLAG_RESET|T37_FLAG_CAL,
		...
		if (test_flag(T37_FLAG_RESUME, &flag)/*||
			...
			dev_info2(dev, "mxt t37 resume msg\n");
			proc_resume_msg(p);
		}

		if (test_flag(T37_FLAG_RESET, &flag)) {
			dev_info2(dev, "mxt t37 reset msg\n");
			proc_reset_msg(p);
		}
		//clear last step
		//clear_flag(T37_STEP_MASK, &obs->step_cmd);
...
static int mxt_proc_step_2_mc_msg(struct plugin_cal *p,unsigned long pl_flag)
{
	const struct mxt_config *dcfg = p->dcfg;
	struct device *dev = dcfg->dev;
	struct t37_observer *obs = p->obs;
	...
	if (ret == 0) {
		...
		if (test_flag(T37_STEP_2_CHECK_REF_2|T37_STEP_2_CHECK_SINGLE_TOUCH, &obs->step)) {
			if (!test_flag(T37_FLAG_T8_SET, &obs->flag)) {
...
static int mxt_proc_step_3_mc_msg(struct plugin_cal *p,unsigned long pl_flag)
{
	const struct mxt_config *dcfg = p->dcfg;
	struct plug_interface *pl = container_of(dcfg, struct plug_interface, init_cfg);
	struct device *dev = dcfg->dev;
	...
	if (test_flag(T37_FLAG_T8_SET, &obs->flag)) {//recovery t8 to last value if set
...
#if defined(CONFIG_MXT_CAL_T37_SC_WORKAROUND)
static int mxt_proc_step_3_sc_msg(struct plugin_cal *p,unsigned long pl_flag)
{
	const struct mxt_config *dcfg = p->dcfg;
	struct plug_interface *pl = container_of(dcfg, struct plug_interface, init_cfg);
	struct device *dev = dcfg->dev;
	struct t37_observer *obs = p->obs;
	struct t37_config *cfg = p->cfg;
	int accu_delta,accu_signal,accu_iron;
	u8 cmd;
	int ret = 0;

	struct surface_count *sc_cnt,*delta_cnt,*signal_cnt,*signal_s_cnt;
	int *sc_drift_coef_list;
	s16 *buf;
	int sc_delta,sc_signal;
	int sc_check_list[2] = {SC_SIGNAL,SC_SIGNAL_S};
	int i,drift,pend_times;
	unsigned long state_move;

	sc_drift_coef_list =  &cfg->sc_drift_coef_array[0];
	sc_delta =  SC_DELTA;
	delta_cnt = &obs->sf_sc_cnt[sc_delta];

	sc_signal= SC_SIGNAL;
	signal_cnt = &obs->sf_sc_cnt[sc_signal];
	signal_s_cnt = &obs->sf_sc_cnt[SC_SIGNAL_S];

	if (delta_cnt->good < signal_cnt->count) {
		buf = obs->sc_buf[SC_DELTA];
		cmd = MXT_T6_DEBUG_DELTA_SC;
		sc_cnt = delta_cnt;
	}else {
		buf = obs->sc_buf[SC_SIGNAL];
		cmd = MXT_T6_DEBUG_SIGNAL_SC;
		sc_cnt = signal_cnt;
	}

	ret = mxt_surface_page_aquire_sc(pl->dev,buf,cmd,&dcfg->m[MX_AA], 1, 0);
	if (ret == -EAGAIN) {
		sc_cnt->retry++;
		dev_dbg(dev, "mxt sc cmd %d (good %d pend %d bad %d retry %d)\n",
				cmd, signal_s_cnt->good,signal_s_cnt->pend,signal_s_cnt->bad,signal_s_cnt->retry);
		return ret;
	} else {
		signal_s_cnt->retry = 0;
		sc_cnt->count++;
	}

	if (signal_cnt->count && delta_cnt->count) {
		if (signal_s_cnt->count == 0) {
			selfcap_merge_range(p, SC_SIGNAL_S, SC_SIGNAL, 0, SURFACE_MERGE_COPY, cfg->sc_acc_flag);
			signal_s_cnt->count++;
		}

		//caculate accumulater for mominal delta and true delta
	
		//true delta accu value
		accu_delta = mxt_surface_sc_statistics(p, pl_flag, SC_TEMP, SC_DELTA, SC_BUFFER_INVALID, &cfg->sc_accu_level_array[0], NUM_SC_LEVEL_LIST, 0, 0);
		//caculated mominal delta accu value, SC_SIGNAL_S is reference
		accu_signal = mxt_surface_sc_statistics(p, pl_flag, SC_TEMP, SC_SIGNAL, SC_SIGNAL_S, &cfg->sc_accu_level_array[0], NUM_SC_LEVEL_LIST, 0, 0);
		if (accu_signal < cfg->sc_accumulator_low) {
			if (test_and_clear_flag(T37_FLAG_SC_SIGNAL, &obs->flag)) {
				if (accu_delta > cfg->sc_accumulator_high)
					signal_cnt->bad++;
				else if (accu_delta > cfg->sc_accumulator_low)
					signal_cnt->pend++;
			}else {
				if (accu_delta > cfg->sc_accumulator_high)
					signal_cnt->pend++;
			}
		} else if (accu_signal > cfg->sc_accumulator_high) {
			set_flag(T37_FLAG_SC_SIGNAL, &obs->flag);
		}

		//drift 
		if (accu_delta < cfg->sc_accumulator_low) {
			if (test_flag(T37_FLAG_SC_IRON, &obs->flag)) {
				if (accu_signal < cfg->sc_accumulator_high)
					selfcap_signal_cache_renew(p, SC_SIGNAL_S, SC_SIGNAL, sc_drift_coef_list[SC_DRIFT_NORMAL], cfg->sc_acc_flag);
			} else{
				drift = SC_DRIFT_SLOW;
				selfcap_signal_cache_renew(p, SC_SIGNAL_S, SC_SIGNAL, sc_drift_coef_list[SC_DRIFT_SLOW], cfg->sc_acc_flag);

				for (i = 0; i < 2; i++) {
					accu_iron = mxt_surface_sc_statistics(p, pl_flag,SC_TEMP,  sc_check_list[i], SC_SIGNAL_IRON, &cfg->sc_accu_level_array[0], NUM_SC_LEVEL_LIST, 0, 0);
					if (accu_iron < cfg->sc_accumulator_low) {
						selfcap_merge_range(p, SC_SIGNAL_S, sc_check_list[i], 0, SURFACE_MERGE_COPY, cfg->sc_acc_flag);
						drift = SC_DRIFT_FAST;
						set_flag(T37_FLAG_SC_IRON, &obs->flag);
					}else if(accu_iron < cfg->sc_accumulator_high) {
						selfcap_merge_range(p, SC_SIGNAL_S, sc_check_list[i], SC_SIGNAL_IRON, SURFACE_MERGE_AVG_HYSIS3, cfg->sc_acc_flag);
						drift = SC_DRIFT_FAST;
					}
				}

				selfcap_signal_cache_renew(p, SC_SIGNAL_S, SC_SIGNAL, sc_drift_coef_list[drift], cfg->sc_acc_flag);
			}
		}
	}

	if (signal_cnt->pend || signal_cnt->bad) {
		state_move = check_touched_and_moving(&obs->trace[0], MAX_TRACE_POINTS, &cfg->movement[MOVEMENT_MIDDLE],NULL,NULL,NULL);

		if (signal_cnt->bad) {
			if (signal_cnt->bad >= cfg->sc_bad_times) {
				dev_info2(dev, "step_3_msg sc signal check accumulator bad set cal (bad %d pend %d)\n",signal_cnt->bad,signal_cnt->pend);
				p->set_t6_cal(p->dev);
			}
		}

		pend_times = signal_cnt->pend + signal_cnt->bad;
		 if (test_flag(T37_STATE_TOUCH_MOVING|T37_STATE_TOUCH_MOV_PENDING, &state_move) && pend_times)
			 pend_times--;
		if (pend_times)
			dev_dbg2(dev, "step_3_msg sc (bad %d pend %d %d mov 0x%lx)\n", 
					signal_cnt->bad,signal_cnt->pend,pend_times,state_move);
		if (pend_times >= cfg->grad_pend_times) {
			dev_info2(dev, "step_3_msg  sc signal check accumulator zero set cal (bad %d pend %d %d mov 0x%lx)\n",signal_cnt->bad,signal_cnt->pend,pend_times,state_move);
			p->set_t6_cal(p->dev);
		}
...
static int mxt_process_obj_recovery(struct plugin_cal *p,unsigned long pl_flag)
{
	...
	if (!test_flag(WK_KEEP_T65_ALWAYS_ON, &cfg->grad_acc_flag)) {
		if ((state == T65_NORMAL) != test_flag(T37_FLAG_T65_SET, &obs->flag)) {
			p->set_t6_cal(p->dev);
		}
	}

	if (state == T65_NORMAL)
		set_flag(T37_FLAG_T65_SET, &obs->flag);
	else
		clear_flag(T37_FLAG_T65_SET, &obs->flag);
...
int config_sc_accu_level_array[NUM_SC_LEVEL_LIST] =
{
	//{ascending order}
	10,25,30,50,70,85,105,130
};

int config_sc_drift_coef_array[SC_DRIFT_NUMBER] = {
	7,
	6,
	4,
};
...
static int init_t37_object(struct plugin_cal *p)
{
	...
	struct t37_config *cfg = p->cfg;
	void *buf;
	int buffer_size,pad_size,sc_size,i;
	...
	sc_size = dcfg->r[SC_Y].end - dcfg->r[SC_X].start;
	sc_size *= sizeof(s16);
	buf = kzalloc(
			buffer_size * (T37_BUFFER_NUMBER + T72_NOISE_STATE_TYPES * 2) +
			sc_size * SC_BUFFER_NUMBER,
			GFP_KERNEL);
	if (!buf) {
	...
	cfg->sc_accu_level_array = config_sc_accu_level_array;
	cfg->sc_drift_coef_array = &config_sc_drift_coef_array[0];

	cfg->sc_accumulator_low = 100;
	cfg->sc_accumulator_high = 200;

	if (dcfg->t104.x.chthr)
		cfg->sc_level_step[SC_X] = dcfg->t104.x.chthr / (NUM_SC_LEVEL_LIST - 2);

	if (dcfg->t104.y.chthr)
		cfg->sc_level_step[SC_Y] = dcfg->t104.y.chthr / (NUM_SC_LEVEL_LIST - 2);

	cfg->shift_sc_drift_low = 7;
	cfg->shift_sc_drift_high = 4;
...
#if defined(CONFIG_MXT_DUMMY_CAL_AT_RESUME)
	cfg->grad_acc_flag |= WK_KEEP_NO_CALIBRAION_AT_RESUME;
#endif
...
static int plugin_cal_t37_store(struct plugin_cal *p, const char *buf, size_t count)
{
	...
	int i,j,config[6]/*,sc,offset,ofs,val,ret*/;
	/*char name[255],type;
	const struct range_t *r; 
	s16 *sc_buf;*/
	...
	if (sscanf(buf, "interval: Dcheck=%ld,Dtmidend=%ld,Dend=%ld,Dend2=%ld,Ggcheck=%ld,Gdcheck=%ld,S0=%ld,S1=%ld,S2=%ld,S3=%ld,S4=%ld,Sf=%ld,Sz=%ld,Sn=%ld,Ss=%ld,S2s=%ld,S2l=%ld",
		...
		if (count > 4 && count < sizeof(name)) {
			...
			dev_dbg(dev, "name %s, offset %d, ret %d\n",name,offset,ret);
			if (ret == 1) {
				if (strncmp(name, "iron", 4) == 0) {
					ret = sscanf(buf + offset, "%c: %n", &type, &ofs);
					dev_dbg(dev, "config (%c), offset %d ret %d^\n",type,ofs,ret);
					if (ret == 2) {
						offset += ofs;
						sc_buf = obs->sc_buf[SC_SIGNAL_IRON];
						r = NULL;
						if (type == 'x') 
							r= &dcfg->r[SC_X];
						else if(type == 'y')
						...
						if (r) {
							for (i = r->start; i <= r->end; i++) {
								...
								if (offset < count) {
									dev_dbg(dev, "%s\n",buf + offset);
									ret = sscanf(buf + offset, "%d %n", 
										&val,&ofs);
									if (ret == 1) {
										sc_buf[i] = (u16)val;
										dev_dbg(dev, "%d",sc_buf[i]);
									}else
										break;
								}else
									break;
							...
							if (i > r->start && ret > 0) {
								print_range_surface(SC_SIGNAL_IRON,NULL,sc_buf,r);
								dev_dbg(dev, "set buf data %d\n", i);
								if (i == r->end)
									set_flag(,obs->flag);
							}
						}
...
struct plugin_cal mxt_plugin_cal_t37 =
{
	...
	.hook_t9 = plugin_cal_t37_hook_t9_t100,
	.hook_t100 = plugin_cal_t37_hook_t9_t100,
