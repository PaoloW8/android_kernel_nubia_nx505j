...
static void plugin_clip_hook_t6(struct plugin_clip *p, u8 status)
{
	...
	dev_info2(dev, "mxt clp flag=0x%lx\n",
		 obs->flag);
}
...
static int plugin_clip_hook_t9_t100(struct plugin_clip *p, int id, int x, int y, struct ext_info *in)
{
	...
	if (!test_flag(CLP_FLAG_FUNC_CLIP|CLP_FLAG_FUNC_SUP, &obs->flag))
		return 0;
...
static int plugin_clip_clp_init(struct plugin_clip *p)
{
	const struct mxt_config *dcfg = p->dcfg;
	struct device *dev = dcfg->dev;
...
static void plugin_clip_clp_deinit(struct plugin_clip *p)
{
	deinit_clp_object(p);
...
struct plugin_clip mxt_plugin_clip = 
{
	.init = plugin_clip_clp_init,
	.deinit = plugin_clip_clp_deinit,
	.start = NULL,
	.stop = NULL,
...
{
	memcpy(p, &mxt_plugin_clip, sizeof(struct plugin_clip));
