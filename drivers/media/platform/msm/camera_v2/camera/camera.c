	...
	struct v4l2_capability *cap)
{
	...
	if (rc < 0) {
		pr_err("%s : posting of MSM_CAMERA_PRIV_QUERY_CAP event failed\n",
					__func__);
	...
	struct v4l2_crop *crop)
{
	...
	if (crop->type == V4L2_BUF_TYPE_VIDEO_CAPTURE_MPLANE) {
		...
		rc = msm_post_event(&event, MSM_POST_EVT_TIMEOUT);
		if (rc < 0) {
				pr_err("%s : posting of MSM_CAMERA_PRIV_S_CROP event failed\n",
					__func__);
		              pr_err("%s : Line %d rc %d\n", __func__, __LINE__, rc);
			return rc;
			}
	...
	struct v4l2_crop *crop)
{
	...
	if (crop->type == V4L2_BUF_TYPE_VIDEO_CAPTURE_MPLANE) {
		...
		rc = msm_post_event(&event, MSM_POST_EVT_TIMEOUT);
		if (rc < 0) {
			pr_err("%s : posting of MSM_CAMERA_PRIV_G_CROP event failed\n",
					__func__);
		       pr_err("%s : Line %d rc %d\n", __func__, __LINE__, rc);
			return rc;
			}
	...
	struct v4l2_queryctrl *ctrl)
{
	...
	if (ctrl->type == V4L2_CTRL_TYPE_MENU) {
		...
		rc = msm_post_event(&event, MSM_POST_EVT_TIMEOUT);
		if (rc < 0) {
			pr_err("%s : posting of MSM_CAMERA_GET_PARM event failed\n",
					__func__);
		       pr_err("%s : Line %d rc %d\n", __func__, __LINE__, rc);
			return rc;
			}
	...
	struct v4l2_control *ctrl)
{
	...
	if (ctrl->id >= V4L2_CID_PRIVATE_BASE) {
		...
		rc = msm_post_event(&event, MSM_POST_EVT_TIMEOUT);
		if (rc < 0) {
			pr_err("%s : posting of MSM_CAMERA_SET_PARM event failed\n",
					__func__);
		       pr_err("%s : Line %d rc %d\n", __func__, __LINE__, rc);
			return rc;
			}
	...
	enum v4l2_buf_type buf_type)
{
	...
	rc = msm_post_event(&event, MSM_POST_EVT_TIMEOUT);
	if (rc < 0) {
		pr_err("%s : posting of MSM_CAMERA_PRIV_STREAM_ON event failed\n",
					__func__);
		pr_err("%s : Line %d rc %d\n", __func__, __LINE__, rc);
		return rc;
		}
		...
		enum v4l2_buf_type buf_type)
{
	...
	rc = msm_post_event(&event, MSM_POST_EVT_TIMEOUT);
	if (rc < 0) {
		pr_err("%s : posting of MSM_CAMERA_PRIV_STREAM_OFF event failed\n",
					__func__);
		pr_err("%s : Line %d rc %d\n", __func__, __LINE__, rc);
		return rc;
		}
	...
	struct v4l2_format *pfmt)
{
	...
	if (pfmt->type == V4L2_BUF_TYPE_VIDEO_CAPTURE_MPLANE) {
		...
		rc = msm_post_event(&event, MSM_POST_EVT_TIMEOUT);
		if (rc < 0) {
			pr_err("%s : posting of MSM_CAMERA_PRIV_G_FMT event failed\n",
					__func__);
		       pr_err("%s : Line %d rc %d\n", __func__, __LINE__, rc);
			return rc;
			}
	...
	struct v4l2_format *pfmt)
{
	...
	if (pfmt->type == V4L2_BUF_TYPE_VIDEO_CAPTURE_MPLANE) {
		...
		rc = msm_post_event(&event, MSM_POST_EVT_TIMEOUT);
		if (rc < 0) {
			pr_err("%s : posting of MSM_CAMERA_PRIV_S_FMT event failed\n",
					__func__);
		       pr_err("%s : Line %d rc %d\n", __func__, __LINE__, rc);
			return rc;
			}
	...
	struct v4l2_streamparm *parm)
{
	...
	rc = msm_post_event(&event, MSM_POST_EVT_TIMEOUT);
	if (rc < 0) {
		pr_err("%s : posting of MSM_CAMERA_PRIV_NEW_STREAM event failed\n",
					__func__);
		       pr_err("%s : Line %d rc %d\n", __func__, __LINE__, rc);
		goto error;
		}
...
static int camera_v4l2_close(struct file *filep)
{
	...
	if (atomic_read(&pvdev->opened) == 0) {
		...
		post_value = msm_post_event(&event, MSM_POST_EVT_TIMEOUT);
               if(post_value < 0) {
				   pr_err("%s : posting of MSM_CAMERA_PRIV_DEL_STREAM event failed\n",
									   __func__);
				   pr_err("%s : Line %d rc %d\n", __func__, __LINE__, post_value);
			   }
