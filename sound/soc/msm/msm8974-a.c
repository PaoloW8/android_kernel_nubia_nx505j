...
#define DRV_NAME "msm8974-asoc-ak4961"
...
#define AK4961_EXT_CLK_RATE 9600000
					...
					bool dapm)
{
	...
	if (enable) {
		if (!codec_clk) {
			dev_err(codec->dev, "%s: did not get AK4961 MCLK\n",
				__func__);
			ret = -EINVAL;
			goto exit;
		}
		...
		if (codec_clk) {
			...
			pr_err("%s: Error setting AK4961 MCLK\n", __func__);
			clk_users--;
			goto exit;
		}
	} else {
		if (clk_users > 0) {
			...
			pr_err("%s: Error releasing AK4961 MCLK\n", __func__);
			ret = -EINVAL;
			goto exit;
		}
	}
...
static int msm8974_ak4961_codec_up(struct snd_soc_codec *codec)
{
	int err;
...
static int msm8974_ak4961_event_cb(struct snd_soc_codec *codec,
		enum ak49xx_codec_event codec_event)
{
	switch (codec_event) {
	case AK49XX_CODEC_EVENT_CODEC_UP:
		return msm8974_ak4961_codec_up(codec);
		break;
	default:
...
*/
	...
	ak4961_event_register(msm8974_ak4961_event_cb, rtd->codec);
	return err;
out:
	clk_put(codec_clk);
	return err;
...
static int msm8974_prepare_codec_mclk(struct snd_soc_card *card)
{
	...
	if (pdata->mclk_gpio) {
		...
		if (ret) {
			dev_err(card->dev,
				"%s: Failed to request ak4961 mclk gpio %d\n",
...
static int msm8974_prepare_us_euro(struct snd_soc_card *card)
{
	...
	if (pdata->us_euro_gpio >= 0) {
		dev_info(card->dev, "%s : us_euro gpio request %d", __func__,
			pdata->us_euro_gpio);
		ret = gpio_request(pdata->us_euro_gpio, "TAIKO_CODEC_US_EURO");
		if (ret) {
			dev_err(card->dev,
				"%s: Failed to request ak4961 US/EURO gpio %d error %d\n",
...
static __devinit int msm8974_asoc_machine_probe(struct platform_device *pdev)
{
	...
	if (pdata->mclk_freq != 9600000) {
		dev_err(&pdev->dev, "unsupported ak4961 mclk freq %u\n",
			pdata->mclk_freq);
		ret = -EINVAL;
		goto err;
	}
...
// Add by wuzehui
    ...
    if(mbhc_switch_enable_gpio == -1 ) {
        ...
        if(mbhc_switch_enable_gpio >= 0){
            ...
            gpio_request(mbhc_cfg.sw_gpio, "headset");
            gpio_direction_output(mbhc_cfg.sw_gpio,0);
            pr_info("headset switch gpio %d and set the value %d\n",
                    mbhc_cfg.sw_gpio,gpio_get_value_cansleep(mbhc_cfg.sw_gpio));
        } else
            mbhc_cfg.sw_gpio = 0;
        pr_info("qcom,mbhc-switch-enable-gpio is %d\n",mbhc_cfg.sw_gpio);
    }
#endif
// end by wuzehui
...
static const struct of_device_id msm8974_asoc_machine_of_match[]  = {
	{ .compatible = "qcom,msm8974-audio-ak4961", },
	{},
};
